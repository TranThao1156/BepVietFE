        ...
        <article className="blog-article">
          ...
          <div className="article-meta">
            <div className="author-mini">
              <img
                src={`http://127.0.0.1:8000/storage/img/NguoiDung/${blog.TacGia.AnhDaiDien}`}
                alt={blog.TacGia.HoTen}
              />
              <div>
                <strong>{blog.TacGia.HoTen}</strong>
                {/* Điều chỉnh Ngày đăng chỉ hiện đ/mm/yyyy */}
                <span>
                  Đăng ngày {new Date(blog.NgayDang).toLocaleDateString()}
                </span>
              </div>
            </div>
            <div className="meta-stats">
              <span>
                <i className="fa-regular fa-comment"></i> {blog.SoBinhLuan} bình
                luận
              </span>
            </div>
          </div>
            ...
            {blog.BlogLienQuan.map((item) => (
              <div className="related-item" key={item.Ma_Blog}>
                ...
                <div className="related-info">
                  <Link to={`/blog/${item.Ma_Blog}`}>{item.TieuDe}</Link>
                </div>
              </div>
            ))}
          </div>
